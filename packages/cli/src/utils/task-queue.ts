// based on https://github.com/BalassaMarton/sequential-task-queue

export type ProgressReporter = () => void;
...
export interface Scheduler {
   ...
   */
  schedule(callback: TaskFunction): void;
}
...
export type EventType = keyof typeof sequentialTaskQueueEvents;
...
export class SequentialTaskQueue {
   ...
   * @param {TaskFunction} task - The function to call when the task is run
   ...
   */
  push(task: TaskFunction, options?: TaskOptions): CancellablePromiseLike<any> {
    if (this._isClosed)
   ...
   */
  on(evt: EventType, handler: Function) {
   ...
   * @param {Function} handler - Event handler. When invoking the handler, the queue will set itself as the `this` argument of the call.
   */
  once(evt: EventType, handler: Function) {
    var cb = (...args: any[]) => {
  ...
  off(evt: EventType, handler: Function) {
    ...
    if (!this.currentTask) {
      ...
      if (task) {
        try {
          ...
          let res = task.callback.apply(undefined, [
  ...
  private doneTask(task: TaskEntry, error?: any) {
    ...
    if (error) {
      ...
      this.emit(sequentialTaskQueueEvents.finished, task);
    }
