...
export class SqliteMigrationProvider implements MigrationProvider {
      ...
      "2022-10-23_base": {
        up: async (db) => {
          ...
          await db.schema
            ...
            .addColumn("created_utc", "datetime")
            .addColumn("tenant_id", "varchar(30)")
            .addColumn("subscriber_info", "jsonb")
            .addColumn("source_subscription", "jsonb")
            ...
            .addColumn("management_urls", "jsonb")
...
export const sqliteMigrateToLatest = (config: SqliteDialectConfig) => {
  const dialect = new SqliteDialect(config);
  const provider = new SqliteMigrationProvider();
  migrateToLatest(dialect, provider);
};
